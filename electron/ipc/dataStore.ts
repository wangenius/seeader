...
/** @Description 插入数据 */
export const db_insert: Motion = (event, datastore: DataStore, query: any) =>
  dataStoreConnector(datastore).insert(query);

export const data = () => {};

data.insert = (datastore: DataStore, query: any) =>
  dataStoreConnector(datastore).insert(query);

data.select = (datastore: DataStore, query: any) =>
  dataStoreConnector(datastore).find(query);
