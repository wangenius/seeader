...
/** @Description 返回导入文件 */
export const file = (path: string): any => window.req(path);

file.read = (path: string) => app(Channels.file_read, path);

/** @Description text保存文件保存 */
file.save = (path: string, content: string) => app("file_write", path, content);
...
file.json_read = <T extends any>(path: string): T => window.req(path);
